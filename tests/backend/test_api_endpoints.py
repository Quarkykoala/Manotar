...
from backend.src.models.models import db, User, KeywordStat
...
def test_keyword_stats_endpoint(client, app, db_session):
    with app.app_context():
        ...
        # Test endpoint
        response = client.get('/api/keyword-stats')
        assert response.status_code == 200
...
def test_keyword_stats_with_filters(client, app, db_session):
    with app.app_context():
        ...
        response = client.get('/api/keyword-stats?department=Engineering')
        assert response.status_code == 200
        data = response.get_json()
        assert len(data) > 0
        
        # Test with location filter
        response = client.get('/api/keyword-stats?location=Mumbai')
        assert response.status_code == 200
        data = response.get_json()
        assert len(data) > 0
        
        # Test with both filters
        response = client.get('/api/keyword-stats?department=Engineering&location=Mumbai')
        assert response.status_code == 200
        data = response.get_json()
        assert len(data) > 0